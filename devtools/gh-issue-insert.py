...
LABEL_RE = re.compile(r"^Labels:\s*(.+)$", re.MULTILINE)
SIZE_RE  = re.compile(r"^Size:\s*(.+)$", re.MULTILINE)
...
# If a Size: line is present, it is applied as an additional label in the
# form "size:<value>" (e.g. "size:M").
#
...
def create_issue(repo, mdfile, project_id=None):
    ...
    if size_match:
        labels.append(f"size:{size_match.group(1).strip()}")
